...
export default function ProjectCard({
  ...
  return (
    <li className="flex min-w-[26rem] max-w-[26rem] min-h-[12rem] max-h-[12rem] rounded-3xl overflow-hidden border border-white max-sm:max-w-[17rem] max-sm:min-w-[17rem]">
      <div className="flex flex-col justify-between w-5/6 h-full px-4 py-4 overflow-hidden whitespace-nowrap overflow-ellipsis">
        <div className="flex flex-col gap-4 overflow-hidden whitespace-nowrap overflow-ellipsis">
          ...
          <div className="flex gap-2 max-sm:gap-2">
            {languageIcons.map((Icon) => (
              <Icon className="text-[2rem] max-sm:text-[1.5rem]" />
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-col items-center justify-start w-1/6 h-full gap-4 pt-4">
        <a href={urlRepo} title="Ir para o repositório">
          <FaGithub className="text-[2rem] max-sm:text-[1.5rem] hover:text-color-primary hover:btn-animation transition-all" />
        </a>
        {urlDeploy && (
          <a href={urlDeploy} title="Ir para o deploy">
            <IoArrowRedoSharp className="text-[2rem] max-sm:text-[1.5rem] hover:text-color-primary hover:btn-animation transition-all" />
          </a>
        )}
      </div>
    </li>
